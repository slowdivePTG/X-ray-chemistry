...
program test_krome
  ...
  integer,parameter::nd=krome_ndust,imax=100,k=krome_idx_HCN
  ...
  write(77,'(a)') "#j21xs "//trim(krome_get_names_header())
  m(:)=get_mass()
  do
     x1(:)=x(:)
     call krome_set_J21xray(j21xs)
     print '(a10,E11.3)',"j21xs:",j21xs
     call krome(x1(:),Tgas,1d8*spy) !call KROME
     x1(:)=max(1d-99*xH,x1(:))
     write(77,'(999E15.5)') log10(j21xs),log10(x1(:)/xH)
     j21xs = j21xs + 0.02
     if(j21xs>4.70) exit
  end do
